...
data = dict(
    ...
    train=dict(
        type='OfftakeDataset',
        ann_file='data/offtake_coco_train.json',
        img_prefix='data/offtake/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations', with_bbox=True),
            dict(type='CutImage'),
            ...
            dict(type='DefaultFormatBundle'),
            dict(
                type='Collect', keys=['img', 'img2', 'gt_bboxes', 'gt_labels'])
        ]),
    val=dict(
        type='OfftakeDataset',
        ann_file='data/offtake_coco_val.json',
        img_prefix='data/offtake/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='CutImage'),
